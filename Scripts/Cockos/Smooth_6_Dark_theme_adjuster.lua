sTitle = 'Smooth_6_Dark theme adjuster'
reaper.ClearConsole()
...
palette = {}
palette.idx = {'Smooth','Pride','Warm','Cool','Vice','Eeek'}
palette.current = tonumber(reaper.GetExtState(sTitle,'paletteCurrent')) or 1
palette.Smooth = {{228,81,78},{210,120,49},{190,178,74},{82,150,70},{69,150,140},{67,122,203},{127,117,227},{167,102,200},{128,128,128},{99,99,99}}
...
function getCurrentPalette()
  return palette.idx[palette.current] or 'Smooth'
end
...
if reaper.LocalizeString ~= nil then
  translate = function(s) return reaper.LocalizeString(s or '---',"Smooth_6_Theme_Adjuster") end
else
...
function switchTheme()
  ...
  if(reaper.file_exists(str.."/Smooth_6_unpacked.ReaperTheme")==true) then
    openTheme = reaper.OpenColorThemeFile(str.."/Smooth_6_unpacked.ReaperTheme")
  else if(reaper.file_exists(str.."/Smooth_6.ReaperThemeZip")==true) then
    openTheme = reaper.OpenColorThemeFile(str.."/Smooth_6.ReaperThemeZip")
    else reaper.ShowConsoleMsg("Smooth 6 theme not found")
    end
  end
  indexParams()
  redraw = 1
...
_switchTheme = Button:new(_wrongTheme, {flow=true,img='button_empty',imgType=3,y=2,w=92,border='x',action=switchTheme,text={str='Switch to the#Smooth v6 theme', align=5,lineSpacing=9,col={129,137,137}}})
